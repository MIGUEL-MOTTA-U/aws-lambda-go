...
- **Etapa 2 (Cognito) — lado front implementado (2026-07-09)**: `auth.ts` (InitiateAuth con USER_PASSWORD_AUTH + refresh, sin SDK), login real en `LoginPage` con fallback simulado sin env vars (`VITE_COGNITO_REGION`, `VITE_COGNITO_CLIENT_ID`, `VITE_COGNITO_USER_POOL_ID`), Bearer (ID token) en todas las peticiones. Pendiente en AWS: User Pool + app client (USER_PASSWORD_AUTH, sin secret), JWT authorizer en rutas de mutación, quitar `ALLOW_UNAUTHENTICATED_UPLOADS`. Guía completa: `real-state-website/docs/AUTH_COGNITO.md`.
